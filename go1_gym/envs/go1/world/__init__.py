...
class World(Navigator):
    def __init__(self, sim_device, headless, num_envs=None, prone=False, deploy=False,
                 cfg: Cfg = None, eval_cfg: Cfg = None, initial_dynamics_dict=None, physics_engine="SIM_PHYSX", locomtion_model_dir = "/common/home/hk919/walk-these-ways/runs/gait-conditioned-agility/pretrain-v0/train/025417.456545"):
    ...
    def compute_reward(self):
        ...
        for i in range(self.num_envs):
            ...
            if dist_to_goal < 0.25:
                goal_reward_temp += GOAL_REWARD * SCALING_FACTOR * 10
        ...
        self.reset_buf |= contact_forces_temp > 0.075
        ...
        if len(self.total_reward_buffer) == 30000:
            self.log_reward('/common/home/hk919/walk-these-ways/rewards/')
    ...
    def log_reward(self, filePath):
        ...
        total_reward_file_name = f'{filePath}total_reward_{self.iter}.csv'
        goal_reward_file_name = f'{filePath}goal_reward_{self.iter}.csv'
        wall_penalty_file_name = f'{filePath}wall_penalty_{self.iter}.csv'
        timeout_penalty_file_name = f'{filePath}timeout_penalty_{self.iter}.csv'
        total_success_file_name = f'{filePath}total_successes_{self.iter}.csv'
        obs_penalty_file_name = f'{filePath}obs_penalty_{self.iter}.csv'
        ...
        obs_penalty_df = pd.DataFrame(self.obs_penalty_buffer)
        ...
        obs_penalty_df.to_csv(obs_penalty_file_name, index=False, header=None, float_format='%.4f')
