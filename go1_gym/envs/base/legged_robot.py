...
class LeggedRobot(BaseTask):
    ...
    def post_physics_step(self):
        ...
        # prepare quantities
        self.base_pos[:] = self.root_states[self.anymal_actor_idxs, 0:3]
        self.base_quat[:] = self.root_states[self.anymal_actor_idxs, 3:7]
        self.base_lin_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[self.anymal_actor_idxs, 7:10])
        self.base_ang_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[self.anymal_actor_idxs, 10:13])
        ...
        self.last_root_vel[:] = self.root_states[self.anymal_actor_idxs, 7:13]
    ...
    def set_idx_pose(self, env_ids, dof_pos, base_state):
        ...
        anymal_actor_idxs_int32 = self.anymal_actor_idxs.to(dtype=torch.int32)
        ...
        if dof_pos is not None:
            ...
            self.gym.set_dof_state_tensor_indexed(self.sim,
                                                  ...
                                                  gymtorch.unwrap_tensor(anymal_actor_idxs_int32[env_ids_long]), len(env_ids_long))

        # base position
        self.root_states[anymal_actor_idxs_int32[env_ids_long]] = base_state.to(self.device)

        self.gym.set_actor_root_state_tensor_indexed(self.sim,
                                                     gymtorch.unwrap_tensor(self.root_states),
                                                     gymtorch.unwrap_tensor(torch.cat((self.anymal_actor_edxs[env_ids_long], self.wall_actor_idxs[env_ids_long]))), 5*len(env_ids_int32))
    ...
    def _reset_dofs(self, env_ids, cfg):
        ...
        all_subject_env_ids = self.anymal_actor_idxs[env_ids].to(device=self.device)
        # wall_env_ids = self.wall_actor_idxs[env_ids].to(device=self.device)
        # all_subject_env_ids = torch.cat((all_subject_env_ids, wall_env_ids))
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        if cfg.env.record_video:
            ...
            self.gym.set_camera_location(self.rendering_camera, self.envs[0], gymapi.Vec3(bx, by, bz + 6.0),
    ...
    def _init_buffers(self):
        ...
        self.last_root_vel = torch.zeros_like(self.root_states[self.anymal_actor_idxs, 7:13])
    ...
    def _render_headless(self):
        if self.record_now and self.complete_video_frames is not None and len(self.complete_video_frames) == 0:
            bx, by, bz = self.root_states[self.anymal_actor_idxs[0], 0], self.root_states[self.anymal_actor_idxs[0], 1], self.root_states[self.anymal_actor_idxs[0], 2]
