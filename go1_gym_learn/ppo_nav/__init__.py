...
class RunnerArgs(PrefixProto, cli=False):
    ...
    # logging
    save_interval = 40  # check for potential saves every this many iterations
...
class Runner:
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False, eval_freq=100, curriculum_dump_freq=500, eval_expert=False):
        ...
        self.env.log_reward('/common/home/hk919/walk-these-ways/runs/gait-conditioned-agility/pretrain-v0/train/025417.456545')
    ...
    def log_reward(self, it, filePath):
        # Create unique file names for each type of reward
        interval = RunnerArgs.save_video_interval
        total_reward_file_name = f'{filePath}total_reward_{it//interval}.csv'
        goal_reward_file_name = f'{filePath}_goal_reward_{it//interval}.csv'
        wall_penalty_file_name = f'{filePath}_wall_penalty_{it//interval}.csv'
        timeout_penalty_file_name = f'{filePath}_timeout_penalty_{it//interval}.csv'
        print('***************************Log Reward**********************')
        print("self.total_reward_buffer:",self.env.total_reward_buffer)
        print('\n\n')
        print('***************************Log Reward**********************')
        print("self.wall_penalty_buffer:",self.env.wall_penalty_buffer)
        print('\n\n')
        print('\n\n')
        print('***************************Log Reward**********************')
        print("self.timeout_penalty_buffer:",self.env.timeout_penalty_buffer)
        print('\n\n')
        print('***************************Log Reward**********************')
        print("self.goal_reward_buffer:",self.env.goal_reward_buffer)

        # Create DataFrames from NumPy arrays
        total_reward_df = pd.DataFrame(self.env.total_reward_buffer)
        goal_reward_df = pd.DataFrame(self.env.goal_reward_buffer)
        wall_penalty_df = pd.DataFrame(self.env.wall_penalty_buffer)
        timeout_penalty_df = pd.DataFrame(self.env.timeout_penalty_buffer)
        
        # Write the updated DataFrames to the CSV files
        total_reward_df.to_csv(total_reward_file_name, index=False, header=None)
        goal_reward_df.to_csv(goal_reward_file_name, index=False, header=None)
        wall_penalty_df.to_csv(wall_penalty_file_name, index=False, header=None)
        timeout_penalty_df.to_csv(timeout_penalty_file_name, index=False, header=None)
